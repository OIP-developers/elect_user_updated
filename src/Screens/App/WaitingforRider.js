...
const WaitingforRider = ({ navigation, route }) => {
    const pickCords=route?.params?.start
    const dropCords=route?.params?.end
    ...
    const [state, setState] = useState({
        pickUpCords: {
            latitude: pickCords?parseFloat(pickCords?.latitude):24.9623677,  /* 24.9623677,67.0463966, */
            longitude:pickCords?parseFloat(pickCords?.longitude):67.0463966,
            latitudeDelta: 0.0922,
            longitudeDelta: 0.0421,
        },
        dropOfCords: {
            latitude: dropCords?parseFloat(dropCords?.latitude):24.9107,
            longitude: dropCords?parseFloat(dropCords.longitude):67.0311,
            latitudeDelta: 0.0922,
            longitudeDelta: 0.0421,
        }
    })
